---
title: "Appwrite"
"og:title": "Appwrite Lead Tracking"
description: "Learn how to track lead conversion events with Appwrite and CodeQR"
---
...
<Steps>
  ...
  <Step title="Create an Appwrite Cloud project">
    ...
    Head to [Appwrite Cloud](https://apwr.dev/appwrite-codeqr) and create a new project.
  ...
  <Step title="Add environment variables">
    ...
    NEXT_CODEQR_API_KEY=<CODEQR_API_KEY>
    ```

  </Step>

  <Step title="Integrate CodeQR Analytics">

    Add the `CodeQRAnalytics` component from the `@codeqr/analytics` package to your app’s root layout.
    ...
    export const metadata: Metadata = {
      title: 'Appwrite CodeQR Leads Example',
      description: 'Appwrite CodeQR Leads Tracking example app with Next.js'
    };
  ...
  <Step title="Set up CodeQR SDK">

    Create the CodeQR client and send leads to CodeQR using the `codeqr.track.lead()` function.

    ```ts src/lib/server/codeqr.ts
    import type { Models } from 'node-appwrite';
    import { CodeQR } from 'codeqr';

    const codeqr = new CodeQR({
      apiKey: process.env.NEXT_CODEQR_API_KEY
    });

    export function addCodeQRLead(user: Models.User<Models.Preferences>, cq_id: string) {
      codeqr.track.lead({
        clickId: cq_id,
        eventName: 'Sign Up',
        externalId: user.$id,
  ...
  <Step title="Send leads to CodeQR on user signup">
    In the `/auth` page, use the Appwrite Admin client to allow users to sign up. Post sign up, check if the `cq_id` cookie is present, send a lead event to CodeQR if found, and delete the `cq_id` cookie.
    ...
    import { addCodeQRLead } from '@/lib/server/codeqr';
    ...
    async function signUpWithEmail(formData: any) {
      ...
      // Check if CodeQR ID is present in cookies and track lead if found
      const cq_id = (await cookies()).get('cq_id')?.value;
      if (cq_id) {
        addCodeQRLead(user, cq_id);
        (await cookies()).delete('cq_id');
      }
...
<Card
  title="Appwrite + CodeQR Next.js  Example"
  icon="github"
  href="https://github.com/appwrite-community/appwrite-codeqr-next"
>
