---
title: "Supabase"
"og:title": "Supabase Lead Tracking"
description: "Learn how to track lead conversion events with Supabase and CodeQR"
---
...
1. In the `/api/auth/callback` route, check if:
   - the `cq_id` cookie is present.
   - the user is a new sign up (created in the last 10 minutes).
2. If the `cq_id` cookie is present and the user is a new sign up, send a lead event to CodeQR using `codeqr.track.lead`
3. Delete the `cq_id` cookie.
...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (!error) {
      ...
      // if the user is new and has a cq_id cookie, track the lead
      if (cq_id && isNewUser) {
        waitUntil(
          codeqr.track.lead({
            clickId: cq_id,
  ...
  if (typeof code === "string") {
    ...
    if (!error) {
      ...
      if (cq_id && isNewUser) {
        ...
        res.setHeader(
          "Set-Cookie",
          `cq_id=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT`
        );
      }
