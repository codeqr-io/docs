...
By default, the script already sets the `cq_id` cookie on a **cross-domain level**.
...
This means that if you have the script installed on a separate domain (e.g. **example.sh**), you can use the `domainsConfig.outbound` prop to ensure that the `cq_id` cookie value is automatically appended to all outbound links targeting your main domain (e.g. **example.com**).
