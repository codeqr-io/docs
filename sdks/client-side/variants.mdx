...
- Detecting the `codeqr_id` query parameter and storing it as a first-party cookie.
...
On top of the features supported by the base variant, it also supports appending the `codeqr_id` cookie to all outbound links targeting the domains you configure.
