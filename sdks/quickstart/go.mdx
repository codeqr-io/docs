...
## 2. Install and initialize the CodeQR Go SDK

<Steps titleSize="h3">

<Step title="Install">

To install the [CodeQR Go SDK](https://codeqr.io/sdks/go), run the following command:
...
Initialize the CodeQR Go SDK by creating a new instance of the `CodeQR` struct.
...
d := codeqr.New(
	codeqr.WithSecurity(os.Getenv("CODEQR_API_KEY")),
)
```
...
Let's create a short link using the [CodeQR Go SDK](https://codeqr.co/sdks/go).
