...
<Note>
  **Plan requirements.** Receiving webhook deliveries is not plan-gated, but the events you can *subscribe* to are. CRUD events, `lead.created`, and `sale.created` require the **Pro** plan or above. High-volume events (`link.clicked`, `qrcode.scanned`, `page.visited`) — and attaching a specific link, QR Code, or Page to a webhook — require the **Business** plan or above. See [Event types](/concepts/webhooks/event-types) for the full breakdown.
</Note>
...
Notifications will be sent to the Project owners at the following intervals:
