...
<Steps>
  ...
  <Step title="Refresh the access token">
    CodeQR access tokens are short-lived, depending on the `expires_in` value (the default value is **604,800 seconds**, or **7 days**). CodeQR will respond with `401 Unauthorized` if you try to use an expired access token.
...
| `conversions.write` | Record lead and sale conversion events on your behalf.          |
